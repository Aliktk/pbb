...
const OFFICES = ['Quetta', 'Pishin', 'Zhob', 'Loralai', 'Chaman', 'Sibi'];
const TOWNS = [
...
const GROUPS = [client_1.BloodGroup.O, client_1.BloodGroup.A, client_1.BloodGroup.B, client_1.BloodGroup.AB];
const RH = [client_1.RhFactor.POSITIVE, client_1.RhFactor.NEGATIVE];
            ...
            for (const rh of RH) {
                await prisma.stockLevel.create({
                    data: {
                        ...
                        bloodGroup: g,
                        rhFactor: rh,
                        unitsAvailable: (g === client_1.BloodGroup.O && rh === client_1.RhFactor.NEGATIVE) ? 1 : (office.length + g.length) % 12,
                    },
                });
            }
        }
    ...
    for (let i = 1; i <= N; i++) {
        ...
        const donor = await prisma.donor.create({
            data: {
                ...
                quantityMl: 350,
                timesDonated: i % 9,
                lastDonatedAt: bucket === 'COOLDOWN' ? ago(30) : bucket === 'ELIGIBLE' ? ago(200) : null,
        ...
        if (bucket === 'REACTIVE') {
            await prisma.screening.create({
                data: {
                    ...
                    hcv: client_1.ScreeningResult.NEGATIVE, hiv: client_1.ScreeningResult.NEGATIVE,
                    hbsAg: client_1.ScreeningResult.POSITIVE, vdrl: client_1.ScreeningResult.NEGATIVE, mp: client_1.ScreeningResult.NEGATIVE,
            ...
            await prisma.screening.create({
                data: {
                    ...
                    hcv: client_1.ScreeningResult.NEGATIVE, hiv: client_1.ScreeningResult.NEGATIVE,
                    hbsAg: client_1.ScreeningResult.NEGATIVE, vdrl: client_1.ScreeningResult.NEGATIVE, mp: client_1.ScreeningResult.NEGATIVE,
                    performedBy: 'Lab', labReference: `LR-${i}`,
                },
            });
        }
